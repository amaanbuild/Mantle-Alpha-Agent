...
from backend.core.security import RateLimiter, parse_amount
...
class BotHandlers:
        ...
        async with session_scope() as session:
            ...
            try:
                ...
                rule = await service.create_rule(
                    user, token, threshold, EventType.ANY, raw_request=update.message.text
                )
    ...
    async def natural_language(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        intent = await self.intent.extract(update.message.text)
        logger.info(
            "bot.intent",
